...
export default async function AdminEditorPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (id === "yeni") return <CourseEditor initial={EMPTY_COURSE} locked={false} isAdmin instructors={instructors} backHref="/admin/kurslar" />;
  const data = await loadCourseForEditor(Number(id));
  if (!data) notFound();
  const { periodEnrolled, ...initial } = data;
  return <CourseEditor key={id} initial={initial} locked={false} isAdmin instructors={instructors} periodEnrolled={periodEnrolled} backHref="/admin/kurslar" />;
}
