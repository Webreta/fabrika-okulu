...
import { db } from "@/db";
import { users, surveyAnswers, surveyCompletions, type Survey } from "@/db/schema";
...
/** Anket sonuçları (eğitmen + admin ortak, server component) — tek anketin sonuçları */
export async function SurveyResults({ survey, base, params, canExport }: { survey: Survey; base: string; params: Record<string, string | undefined>; canExport: boolean }) {
  ...
  const doneExists = sql`exists (select 1 from ${surveyCompletions} sc where sc.user_id = ${users.id} and sc.survey_key = ${survey.key})`;
  const conds = [eq(users.role, "student")];
  if (s) conds.push(or(ilike(users.firstName, `%${s}%`), ilike(users.lastName, `%${s}%`), ilike(users.email, `%${s}%`))!);
  if (durum === "done") conds.push(doneExists);
  if (durum === "never") conds.push(sql`not ${doneExists}`);
  for (const [qk, val] of filters) {
    conds.push(sql`exists (select 1 from ${surveyAnswers} sa where sa.user_id = ${users.id} and sa.survey_key = ${survey.key} and sa.question_key = ${qk} and (sa.value = to_jsonb(${val}::text) or sa.value @> to_jsonb(array[${val}::text])))`);
  }
  const [[cnt], list, doneSet] = await Promise.all([
  ...
  const [all] = await db.select({ n: sql<number>`count(*)`.mapWith(Number) }).from(users).where(eq(users.role, "student"));

  const uid = Number(params.uid);
  if (uid) {
    const [u] = await db.select().from(users).where(eq(users.id, uid)).limit(1);
    const ans = await db.select().from(surveyAnswers).where(and(eq(surveyAnswers.userId, uid), eq(surveyAnswers.surveyKey, survey.key)));
    return (
      <div className="card">
        ...
        {canExport && u && <div className="mt-2"><ResetSurveyButton userId={u.id} surveyKey={survey.key} /></div>}
  ...
  const filterable = survey.questions.filter((q) => (q.type === "radio" || q.type === "checkbox") && q.options?.length);
  ...
  return (
    <div className="space-y-5">
      <div className="grid grid-cols-2 gap-4 md:grid-cols-4">
        ...
        <Kpi label="Anketi tamamladı" value={doneSet.size} icon="check" color="green" />
        <Kpi label="Bekliyor" value={all.n - doneSet.size} icon="clock" color="amber" />
        <Kpi label="Tamamlama oranı" value={`%${all.n ? Math.round((doneSet.size / all.n) * 100) : 0}`} icon="chart" color="sky" />
      </div>
      <form className="card grid gap-3 md:grid-cols-4" method="get">
        ...
        <div className="flex gap-2 md:col-span-4">
          ...
          {canExport && <a href={`/api/admin/anket-csv?anket=${survey.id}&${new URLSearchParams(Object.entries(params).filter(([, v]) => v) as [string, string][]).toString()}`} className="btn-secondary btn-sm ml-auto">CSV indir</a>}
        </div>
      </form>
          ...
          <tbody>
            ...
            {list.map((u) => (
              <tr key={u.id}>
                ...
                <td>{doneSet.has(u.id) ? <Chip color="green">Tamamladı</Chip> : <Chip color="gray">Doldurmadı</Chip>}</td>
              </tr>
            ))}
          </tbody>
        </table>
