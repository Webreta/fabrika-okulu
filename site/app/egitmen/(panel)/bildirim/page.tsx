...
export default async function TeacherNotificationsPage({ searchParams }: { searchParams: Promise<{ p?: string }> }) {
  ...
  return (
    <>
      ...
      <NotificationList items={rows.map((n) => ({ id: n.id, title: n.title, body: n.body, url: n.url, read: n.read, createdAt: n.createdAt.toISOString() }))} unread={unread} />
      <Pagination page={page} totalPages={totalPages} basePath="/egitmen/bildirim" />
    </>
  );
}
