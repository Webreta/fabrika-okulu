...
import { saveCertificateTemplate, deleteCertificateTemplate, uploadCertificateImage } from "@/app/actions/admin";
    ...
    <div className="grid gap-6 xl:grid-cols-[1fr_360px]">
      <div className="space-y-4">
        ...
        <div ref={canvasRef} className="relative cursor-crosshair overflow-hidden rounded-xl border border-line shadow" onPointerMove={onMove} onPointerUp={() => (drag.current = null)} onPointerLeave={() => (drag.current = null)}>
          <CertificateCanvas imageUrl={t.imageUrl} imageWidth={t.imageWidth} imageHeight={t.imageHeight} fields={t.fields} name={t.sampleName} course={t.sampleCourse} date={today} qrDataUrl={t.fields.qr.enabled ? "data:image/svg+xml;utf8," + encodeURIComponent('<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 10 10"><rect width="10" height="10" fill="#fff"/><rect x="1" y="1" width="3" height="3"/><rect x="6" y="1" width="3" height="3"/><rect x="1" y="6" width="3" height="3"/><rect x="6" y="6" width="1" height="1"/><rect x="8" y="8" width="1" height="1"/></svg>') : null} />
      ...
      <aside className="space-y-4">
        ...
        <div className="card space-y-3 text-sm">
          ...
          <p className="text-xs text-muted">Sertifika otomatik verilmez; eğitmen koşulu sağlayan öğrenciye panelden tanımlar.</p>
        </div>
        {msg && <p className="rounded-lg bg-sky-50 px-3 py-2 text-sm">{msg}</p>}
        <button disabled={pending} onClick={() => start(async () => { const r = await saveCertificateTemplate(t); if (r.ok) { setMsg("Kaydedildi."); if (!t.id && r.id) router.replace(`/admin/sertifikalar/${r.id}`); router.refresh(); } else setMsg(r.error); })} className="btn-primary w-full"><Icon name="save" className="size-4" /> {pending ? "…" : "Kaydet"}</button>
      </aside>
    </div>
  );
}
