            ...
            <div key={`${i.kind}-${i.id}`} className="card flex flex-col">
              <div className="flex items-start justify-between gap-2">
                ...
                <span className="flex items-center gap-1.5">
                  <Chip color={i.kind === "quiz" ? "sky" : "amber"}>{i.kind === "quiz" ? "Sınav" : "Görev"}</Chip>
              ...
              {i.due && <p className="mt-2 mb-4"><span className={`date-chip ${!i.done && i.due.getTime() < Date.now() ? "bg-red-100 text-red-700" : ""}`}>Son tarih: {fmtDateTime(i.due)}</span></p>}
              <Link href={i.link} className="btn-secondary btn-sm mt-auto self-start">{i.kind === "quiz" ? (i.done ? "Sonucu gör" : "Sınava başla") : "Göreve git"}</Link>
            </div>
          ))}
        </div>
      )}
