...
/**
 * Yeni yayınlanan (ve kullanıcının doldurmadığı) anket için popup.
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-navy-900/70 p-4">
      <div className="w-full max-w-md rounded-2xl bg-white p-6 text-center shadow-2xl">
        ...
        <h2 className="mt-4 text-xl font-bold text-navy-800">Yeni anket: {survey.title}</h2>
        ...
        <div className="mt-6 flex justify-center gap-2">
          ...
          <button onClick={() => { setOpen(false); router.push(`/panel/anket/${survey.id}`); }} className="btn-primary">Ankete git</button>
        </div>
      </div>
    </div>
  );
