...
import { PageTitle, Chip, Tabs } from "@/components/panel/ui";
import { IssueCertButton, RevokeCertButton } from "@/components/teacher/IssueCertButton";

export default async function AdminIssueCertificatesPage({ searchParams }: { searchParams: Promise<{ course?: string }> }) {
  ...
  return (
    <>
      ...
      {feed.templates.length === 0 ? (
        <p className="card text-muted">Henüz sertifika tasarımı yok. <Link href="/admin/sertifikalar/yeni" className="text-sky-600 underline">Tasarım oluştur</Link></p>
      ) : (
                ...
                <tr key={`${r.userId}-${r.courseId}`}>
                  ...
                  <td>
                    <div className="flex flex-wrap items-center gap-2">
                      {r.issued.map((i) => <span key={i.id} className="flex items-center gap-1 text-xs"><a href={`/sertifika/${i.token}`} target="_blank" className="text-sky-600 underline">{i.title}</a><span className="text-muted">({fmtDate(i.issuedAt)})</span><RevokeCertButton id={i.id} /></span>)}
