...
export default async function DocumentsPage() {
  ...
  return (
    <>
      <PageTitle title="Belge Yükle" sub="Öğrenci / yeni mezun indirimi için belgeni yükle, sana özel kupon tanımlayalım." />
      <div className="grid gap-6 lg:grid-cols-2">
        <div className="card">
          <h2 className="mb-1 font-bold text-navy-800">Yeni belge</h2>
          <p className="mb-4 text-sm text-muted">Öğrenci belgesi veya diploma · PDF/JPG/PNG · en fazla 10 MB</p>
          <DocumentUploadForm />
        </div>
        <div className="card">
                  ...
                  <li key={d.id} className="py-3">
                    <div className="flex items-start justify-between gap-3">
                      <div className="min-w-0">
                        ...
                        <p className="text-xs text-muted">{fmtDate(d.createdAt)}{d.note && ` · ${d.note}`}</p>
                      </div>
